 ...
 * Info 下拉刷新和加载更多的View
 */
public class ALPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private ALListViewBaseHeader mHeaderView;
    private ALListViewHeader defaultViewHeader;

    /** footer view. */
	private ALListViewFooter mFooterView;
	 ...
	 */
	public ALPullToRefreshView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	 ...
	 * @param context the context
	 */
	public ALPullToRefreshView(Context context) {
		super(context);
		init(context);
	}
	...
	private void init(Context context) {
        ...
        defaultViewHeader = new ALListViewHeader(mContext);
		setHeaderView(defaultViewHeader);
	}

    public ALListViewHeader getDefaultHeaderView() {
	 ...
	 * 增加头布局.
	 */
	public void setHeaderView(ALListViewBaseHeader headerView) {

        this.mHeaderView = headerView;
		//获取View的高度
//		ISViewUtil.measureView(this.mHeaderView);
	...
	private void addFooterView() {
        ...
        mFooterView = new ALListViewFooter(mContext);
...
//             linearManager = null;
			...
			if (layoutManager instanceof LinearLayoutManager) {
				...
				lastVisibleItems = staggeredGridLayoutManager.findFirstCompletelyVisibleItemPositions(lastVisibleItems);
			...
			if (deltaY > 0) {
				...
				if (firstItemPosition == 0 && child.getTop() == 0) {
					mPullState = PULL_DOWN_STATE;
					return true;
				}
				int top = child.getTop();
				int padding = mRecyclerView.getPaddingTop();
				if (firstItemPosition == 0 && Math.abs(top - padding) <= 11) {
					mPullState = PULL_DOWN_STATE;
					return true;
				}
				...
				if (lastChild.getBottom() <= getHeight() && lastItemPosition == layoutManager.getItemCount() - 1) {
					mPullState = PULL_UP_STATE;
					return true;
				}
			}
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != ALListViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(ALListViewHeader.STATE_READY);

		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(ALListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != ALListViewFooter.STATE_LOADING) {
			mFooterView.setState(ALListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(ALListViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(ALListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(ALListViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(ALListViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(ALListViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(ALListViewFooter.STATE_READY);
		}
     ...
     */
    public ALListViewBaseHeader getHeaderView() {
        return mHeaderView;
    }
     ...
     */
    public ALListViewFooter getFooterView() {
        return mFooterView;
    }
	...
	public interface OnFooterLoadListener {
		 ...
		 * @param view the view
		 */
		public void onFooterLoad(ALPullToRefreshView view);
	}
	...
	public interface OnHeaderRefreshListener {
		 ...
		 * @param view the view
		 */
		public void onHeaderRefresh(ALPullToRefreshView view);
	}
