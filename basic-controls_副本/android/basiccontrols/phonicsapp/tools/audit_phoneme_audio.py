...
GENERATION_RECORD = TOOLS / "audio_generation_record.json"
AUDIO_ROOTS = [
...
def source_for(pid: str, gen, generation_record: dict) -> str:
    recorded_source = generation_record.get("phonemes", {}).get(pid)
    if recorded_source:
...
def risk_for(pid: str, symbol: str, duration: float, gen, source: str) -> str:
    if "User-provided" in source:
        return "needs-license-record: user-provided source"
    if pid == "zh":
...
def main() -> int:
    ...
    generation_record = json.loads(GENERATION_RECORD.read_text(encoding="utf-8"))
    ...
    for item in entries:
        ...
        source = source_for(pid, gen, generation_record)
        if source == "UNKNOWN":
        ...
        rows.append(
            {
                ...
                "size": size,
                "source": source,
                "status": risk_for(pid, symbol, duration, gen, source),
            }
        )
    ...
    if errors:
        ...
        return 1
    print(f"PASS: {len(rows)} phonemes audited; {len(risky)} items require manual handling")
    print(f"report: {REPORT}")
    return 0
