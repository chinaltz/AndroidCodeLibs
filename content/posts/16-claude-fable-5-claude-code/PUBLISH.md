...
> 审核时间：2026-06-10  
> 文章路径：`content/posts/16-claude-fable-5-claude-code/index.md`
...
3. **跑导出命令**：`npm run publish:post -- content/posts/16-claude-fable-5-claude-code`
...
2. 删除正文中两段 `📁` 提示词块
3. 确认主标题（见下）
4. 执行 `npm run publish:post -- content/posts/16-claude-fable-5-claude-code`
...
```bash
# 1. 预览图链替换（可选）
npm run upload:images -- content/posts/16-claude-fable-5-claude-code --dry-run

# 2. 正式上传 + 导出 HTML/DOCX/published.md
npm run publish:post -- content/posts/16-claude-fable-5-claude-code

# 仅本地预览（不上传图）
npm run convert -- content/posts/16-claude-fable-5-claude-code
```
