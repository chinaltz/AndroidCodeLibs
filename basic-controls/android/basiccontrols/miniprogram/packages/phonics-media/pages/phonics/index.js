...
Page({
  ...
  onPhonemeTap(e) {
    ...
    const app = getApp();
    const path = audio.phonemePath(id);
    audio.play(path, phoneme.symbol).catch((err) => {
      console.error('phonics list audio failed', {
        phonemeId: id,
        path,
        error: err,
      });
  ...
  onStartLearn() {
    nav.navigateTo(`/packages/phonics-media/pages/learn/index?id=${this.data.currentId}`);
  },
